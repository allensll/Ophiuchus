...
import utils
from PseudoNoise.nn.resnet_sm import resnet32, resnet56
from PseudoNoise.nn.fixup_resnet_sm import fixup_resnet32, fixup_resnet56
...
def train(args, model, device, train_loader, optimizer, epoch):
    ...
    for batch_idx, (data, target) in enumerate(train_loader):
        ...
        if args.noise is None:
            ...
            loss2 = utils.contrastive_loss2(output_, target, 100)
...
def main():
    ...
    train_loader, test_loader = utils.load_CIFAR100(args.batch_size, test_batch_size=1000, new=True, num_workers=4)

    nf = True
    if nf:
        model = fixup_resnet32(pretrained=False, num_classes=100, width=args.width).to(device)
        ...
        path = '../pretrained/acc_fixup_resnet32-{}-{}_c100.pkl'.format(args.width, args.lam)
    else:
        model = resnet32(pretrained=False, num_classes=100, width=args.width).to(device)
        optimizer = optim.SGD(model.parameters(), args.lr, momentum=0.9, weight_decay=args.wd)
        path = '../pretrained/acc_resnet32-{}_c100.pkl'.format(args.width)
...
#         with utils.timer('Epoch '):
    ...
    if args.save_pretrained:
        torch.save(model.state_dict(), '../pretrained/fixup_resnet32-{}_c100.pt'.format(args.width))
...
def acc():
    ...
    train_loader, test_loader = utils.load_CIFAR100(128, test_batch_size=test_batch_size, **kwargs)
    model = resnet56(pretrained=True)
