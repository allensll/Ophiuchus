...
def train(args, model, device, train_loader, optimizer, epoch):
    ...
    for batch_idx, (data, target) in enumerate(train_loader):
        ...
        if args.noise is None:
            ...
            loss2 = utils.contrastive_loss2(output_, target, 2)
...
def main():
    ...
    parser.add_argument('--gamma', type=float, default=5, metavar='M',
                        help='reduce the fixup learning rate (default: 5)')
    ...
    train_loader, test_loader = utils.load_CelebA(args.batch_size, test_batch_size=1000, new=True, num_workers=4)

    nf = True
    if nf:
        model = fixup_resnet32(pretrained=False, num_classes=2, width=args.width).to(device)
        parameters_bias = [p[1] for p in model.S.named_parameters() if 'bias' in p[0]]
        parameters_scale = [p[1] for p in model.S.named_parameters() if 'scale' in p[0]]
        parameters_u = model.U.parameters()
        parameters_others = [p[1] for p in model.S.named_parameters() if not ('bias' in p[0] or 'scale' in p[0])]
        optimizer = optim.SGD(
            [{'params': parameters_bias, 'lr': args.lr/5.},
             {'params': parameters_scale, 'lr': args.lr/5.},
             {'params': parameters_u, 'lr':args.lr/5.},
             {'params': parameters_others}],
            lr=args.lr,
            momentum=0.9,
            weight_decay=args.wd)
        path = '../pretrained/acc_nf_resnet20-{}-{}_cb.pkl'.format(args.width, args.lam)
    else:
        model = resnet32(pretrained=False, num_classes=2, width=args.width).to(device)
        optimizer = optim.SGD(model.parameters(), args.lr, momentum=0.9, weight_decay=args.wd)
        path = '../pretrained/acc_resnet20-{}_cb.pkl'.format(args.width)
    ...
    if args.save_pretrained:
        torch.save(model.state_dict(), '../pretrained/nf_resnet20-{}_cb.pt'.format(args.width))
