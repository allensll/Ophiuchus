...
from .fixup_resnet_sm import fixup_resnet20, fixup_resnet32, fixup_resnet56, fixup_resnet110
...
def construct_model(model, num_classes=10, seed=None, modelkey=None):
    ...
    if model == 'ResNet20':
        ...
        model = resnet20(pretrained=False, num_classes=num_classes, width=1)
    elif model == 'ResNet20-fixup':
        model = fixup_resnet20(pretrained=False, num_classes=num_classes, width=1)
    elif model == 'ResNet32-fixup':
        model = fixup_resnet32(pretrained=False, num_classes=num_classes, width=1)
    elif model == 'ResNet56-fixup':
        model = fixup_resnet56(pretrained=False, num_classes=num_classes, width=1)
    elif model == 'ResNet110-fixup':
        model = fixup_resnet110(pretrained=False, num_classes=num_classes, width=1)
    elif model == 'LeNet5':
