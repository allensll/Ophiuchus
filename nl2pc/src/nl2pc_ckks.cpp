...
std::vector<double> NLParty::cmp(const std::vector<double>& data) {
    ...
    if (!input2.empty()) {
        ...
        for (std::size_t i=0; i<n2; i++) {
            ...
            k[i] = static_cast<double>(rand() % 1000000 + 500000) / 1000000;
        }
...
//        boost::asio::write(socket, boost::asio::buffer(res1), boost::asio::transfer_exactly(sizeof(double)*n1));
        this->write_finish = false;
        boost::asio::async_write(socket, boost::asio::buffer(res1), boost::asio::transfer_exactly(sizeof(double)*n1), std::bind(&NLParty::handler, this, std::placeholders::_1, std::placeholders::_2));
    }
    // step 4
    if (!input2.empty()) {
...
std::vector<double> NLParty::max(const std::vector<double>& input, const std::size_t patch_size) {
    ...
    std::vector<seal::Ciphertext> en_data1(n_cipher1);
    std::vector<seal::Ciphertext> en_data2(n_cipher2);
    std::vector<double> res1(n1_);
    std::vector<double> res2(n2_);
    ...
    if (!input2.empty()) {
        ...
        for (std::size_t i=0; i<n2_; i++) {
            // using rand() insecurity
            double rd = static_cast<double>(rand() % 1000000 + 5000000) / 1000000;
    ...
    if (!input1.empty()) {
        ...
        for (std::size_t i=0; i<n1_; i++) {
            ...
            for (std::size_t j=1; j<patch_size; j++) {
                if (a[max_idx*n1_+i] < a[j*n1_+i]) {
                    max_idx = j;
                }
            }
            res1[i] = max_idx;
        }
        boost::asio::write(socket, boost::asio::buffer(res1), boost::asio::transfer_exactly(sizeof(double)*n1_));
    }
    // step 4
    if (!input2.empty()) {
        if (this->verbose) {std::cout << "setp 4" << std::endl;}
        boost::asio::read(socket, boost::asio::buffer(res2), boost::asio::transfer_exactly(sizeof(double)*n2_));
    }
    // merge two vectors
