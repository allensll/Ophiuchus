"""Mechanisms for image reconstruction from feature gradients."""
import itertools
...
def register_hook(model, grads, mtype):
    handles = list()

    def bk_hook(module, grad_input, grad_output):
        grads.append(grad_output[0])
    for l in model.S.modules():
        if isinstance(l, nn.Conv2d) or isinstance(l, nn.Linear) or isinstance(l, nn.MaxPool2d) or isinstance(l, nn.ReLU):
            handles.append(l.register_full_backward_hook(bk_hook))
    if mtype == 'F':
        for l in model.U.modules():
            if isinstance(l, nn.Conv2d) or isinstance(l, nn.Linear) or isinstance(l, nn.MaxPool2d) or isinstance(l, nn.ReLU):
                handles.append(l.register_full_backward_hook(bk_hook))
    return handles


def register_hook2(model, features, mtype):
    handles = list()

    def fw_hook(module, args, output):
        features.append(output)
    for l in model.S.modules():
        if isinstance(l, nn.Conv2d) or isinstance(l, nn.Linear) or isinstance(l, nn.MaxPool2d) or isinstance(l, nn.ReLU):
            handles.append(l.register_forward_hook(fw_hook))
    if mtype == 'F':
        for l in model.U.modules():
            if isinstance(l, nn.Conv2d) or isinstance(l, nn.Linear) or isinstance(l, nn.MaxPool2d) or isinstance(l, nn.ReLU):
                handles.append(l.register_forward_hook(fw_hook))
...
def ExtractFeGr(model, loss_fn, ground_truth, labels, mtype):
    model.eval()
    model.zero_grad()
    grads = list()
    handles = register_hook(model, grads, mtype)
    target_loss = loss_fn(model(ground_truth), labels)
    target_loss.backward()
    input_gradient = [grad.detach() for grad in grads]
    for h in handles:
        h.remove()
    return input_gradient


def ExtractFeGr_mod1(model, loss_fn, ground_truth, labels, mtype, sig_):
    model.eval()
    model.zero_grad()
    ...
    def hook(grad):
        # sig = sig_ * 1e-3
        m = Normal(torch.tensor([0.0]), torch.tensor([sig_]))
        n = m.sample(grad.size()).squeeze(-1)
        grad_ = grad + n.to('cuda:0')
        return grad_
    output.register_hook(hook)

    target_loss = F.cross_entropy(output, labels)
    # target_loss = loss_fn(model(ground_truth), labels)
    target_loss.backward()
    input_gradient = [grad.detach() for grad in grads]
    for h in handles:
...
class FeGrReconstructor:
    ...
    def _init_images(self, img_shape):
        if self.config['init'] == 'randn':
            return torch.randn((self.config['restarts'], self.num_images, *img_shape), **self.setup) + 0.5
        elif self.config['init'] == 'rand':
            return torch.rand((self.config['restarts'], self.num_images, *img_shape), **self.setup)
    ...
    def _run_trial(self, x_trial, input_data, gt_labels, dryrun=False):
        ...
        if self.mtype == 'S':
            ...
            # labels = torch.nn.functional.one_hot(gt_labels, output_test.shape[1])*20-10
            # labels = gt_labels.to(**self.setup).requires_grad_(True)
            labels = gt_labels
    ...
    def _gradient_closure(self, optimizer, x_trial, input_gradient, label):
        ...
        def closure():
            ...
            features = list()
            handles = register_hook2(self.model, features, self.mtype)
            optimizer.zero_grad()
            ...
            for h in handles:
                h.remove()
            features.reverse()
            gradient = torch.autograd.grad(loss, features, create_graph=True)
    ...
    def _average_trials(self, x, labels, input_data, stats):
        ...
        self.pre.zero_grad()
        x_optimal.grad = None
        x_trial_ = self.pre(x_optimal)
        loss = self.loss_fn(self.model(x_trial_), labels)
        for h in handles:
            h.remove()
        features.reverse()
        gradient = torch.autograd.grad(loss, features, create_graph=True)
