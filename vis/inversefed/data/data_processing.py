...
def construct_dataloaders(dataset, defs, data_path='~/data', shuffle=True, normalize=True):
    ...
    if dataset == 'CIFAR10':
        ...
        trainset, validset = _build_fashion_mnist(path, defs.augmentations, normalize)
        loss_fn = Classification()
    elif dataset == 'CelebA':
...
def _build_celeba(data_path, augmentations=True, normalize=True):
    ...
    # Load data
    print(data_path)
    trainset = torchvision.datasets.ImageFolder(os.path.join(data_path, 'CelebA', 'train'), transform=transforms.ToTensor())
    validset = torchvision.datasets.ImageFolder(os.path.join(data_path, 'CelebA', 'test'), transform=transforms.ToTensor())
...
def _build_fashion_mnist(data_path, augmentations=True, normalize=True):
    """Define Fashion-MNIST with everything considered."""
    # Load data
